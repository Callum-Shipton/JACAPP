	...
	public void move(Vector2 moveVec) {
		...
		for(int i = minX; i <= maxX; i++){
			for(int j = minY; j <= maxY; j++){
				if(i >= 0 && j >= 0 && i <= ShootEmUp.currentLevel.getWidth() && j <= ShootEmUp.currentLevel.getHeight()){
					wall = ShootEmUp.currentLevel.walls.get((j * ShootEmUp.currentLevel.getHeight()) + i);
					if(wall != null){
