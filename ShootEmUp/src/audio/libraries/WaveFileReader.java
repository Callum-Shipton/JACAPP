...
public class WaveFileReader extends SunFileReader {
	...
	public AudioFileFormat getAudioFileFormat(InputStream stream) throws UnsupportedAudioFileException, IOException {
		...
		// InputStream, but works for URL
		AudioFileFormat aff = getFMT(stream, true);
		// the following is not strictly necessary - but was implemented like
		// that in 1.3.0 - 1.4.1
	 ...
	 * point to valid audio file data. In general, audio file providers may need
	 * to read some data from the stream before determining whether they support
	...
	public AudioInputStream getAudioInputStream(InputStream stream) throws UnsupportedAudioFileException, IOException {
		...
		AudioFileFormat fileFormat = getFMT(stream, true); // throws
	...
	private AudioFileFormat getFMT(InputStream stream, boolean doReset)
		...
		while (true) {
			...
			try {
				...
				// we've reached the end of the file without finding the 'fmt'
				// chunk
		...
		if (wav_type == WaveFileFormat.WAVE_FORMAT_PCM) {
			encoding = AudioFormat.Encoding.PCM_SIGNED; // if 8-bit, we need
														// PCM_UNSIGNED,
														// below...
		...
		// $$kk: if "data" chunk precedes "fmt" chunk we are hosed -- can this
		// legally happen?
		nread = 0;
		while (true) {
			try {
				...
				// we've reached the end of the file without finding the 'data'
				// chunk
				throw new UnsupportedAudioFileException("Not a valid WAV file");
			}
		}
		...
		AudioFormat format = new AudioFormat(encoding, sampleRate, sampleSizeInBits, channels,
				calculatePCMFrameSize(sampleSizeInBits, channels), sampleRate, false);
